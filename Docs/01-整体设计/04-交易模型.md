...
- **窗内修改的通道分工（数值折叠 vs 结构直写）**：§2 的三类可改对象按字段形状各走各路，互不越界——
  ...
  - **接缝（响应者如何拿到 `Tx`）**：窗口负载即交易引用（`03` §4）。服务端技能组件由
    `OnCanActivate(Timing, Tx)` 被询问——**签名已落**（携带负载；`AskNextResponder` 传 `ActiveWindow->Tx`），
    这是技能在窗内拿交易引用的**唯一途径**（钩子职责 = 就位而非产修改，见 `09` §6 与 `13` §4.1）。
    客户端主导的结构修改（人手选换目标）随下发/提交 RPC 协议定形（`11`/P7）；
...
- **子系统交易队列**：
  ...
  - **嵌套栈与入口已落（2026-09-16）**：`NestedStack`（`TArray<FNestedTransactionFrame>`，帧内两字段同为强引用）
    ...
    `EnqueueNested(Child)`）、`PopNestedTransaction`（栈顶自校 + 弹栈 + 换手回父）均已落；`OnTransactionFinished`
  ...
  - **事件化委托**：7 条声明与广播点**全部落地**（2026-09-18 复核）——`FWindowOpened`/`FWindowClosed`/
    `FTransactionModified`/`FResponseLockChanged` + `FTransactionEnqueued`（入队两路径：`EnqueueTransaction`
    忙锁分支前 / `EnqueueNested`）/ `FTransactionFinished`（`OnTransactionFinished`，实例名带 `Event` 后缀）/
    `FTransactionNested`（子进 `PushNestedTransaction` + 子出 `OnTransactionFinished` 栈顶分支，两端点配对）；
    内部推进本身仍为 C 面直调（`Finish` 内调 `BroadcastFinish` + `OnTransactionFinished`），事件化委托是旁路观察位
    （`12` §5.2/§7/§9）。
