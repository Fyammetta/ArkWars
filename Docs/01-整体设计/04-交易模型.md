...
- **`QueryTimings()`** — 调用方：`Drive()`（P3 起；现为占位，置 `Querying`）。
  职责：逐个开 `03` 查询时机窗口。
- **`OnWindowClosed()`** — 调用方：窗口关闭（P3；现为空壳）。
  职责：汇总结果 → 有可落子结果走 `Execute`；被防止/无效直接 `Finish(false)`（不 Execute）。
- **`Drive()`** — 调用方：管理器（`DriveTransaction`）。**驱动器唯一入口，非虚**（09-10 定案）：
  当前直调族钩子 `Execute()`；P3 起改调 `QueryTimings()` 开窗——入口名与调用方不变，
...
- **入口方法非虚，族差异只走虚钩子（09-08 决策；09-10 补 `Drive`）**：
  `Start/Drive/QueryTimings/OnWindowClosed/Finish` 是闭环入口，不整开虚——族覆写入口会破坏门控
  统一与 §6 "新增交易不碰入口"的扩展契约。**驱动器只认 `Drive()`**（09-10 定案）：族钩子
  `Execute()` 收回 `protected`，`Drive()` 体是唯一允许调它的位置（P3 起改调 `QueryTimings()`）。
...
DriveTransaction()         // 忙则直接返回；出队首置 Active 并驱动（驱动 = Active->Drive()，09-10）
...
- **"通知加入"的完整链路（谁在何时调 `AppendModification`）**：ModRequest **不是被推送进交易的**，
  ...
  现状缺口见 `12` §4；现状 `QueryTimings` 只置 `Querying`、无开窗无通知，故
  `AppendModification` 暂无调用者（P3 补齐）。
...
- **`Battle/Transaction/BattleTransaction.h/.cpp`（+ `EventTypes.h`/`EventTags.h`）** —
  ...
  - `Execute()` 为**受保护** `PURE_VIRTUAL`（09-10 定案：原公开 → 收回 `protected`；驱动器改经
  ...
  - `Instigator`/`Targets` 公开字段（`TWeakObjectPtr<AActor>`/`TArray<TWeakObjectPtr<AActor>>`）
    ——**类型链已定稿 `AActor*`**（09-10，P2 §2-F 收口，原 `APlayerState*` 草稿作废）；
...
- **`UCardMoveTransaction`** — 独立文件（`Transaction/SubTransaction/CardMoveTransaction.*`）：
  ...
  - 族实现已落（09-09：`Execute` 执行骨架 + `Validate` + `FoldMods` 直通；09-10 九次复核：
    `Execute` 收回 `protected`、`BroadcastFinish` 真广播 `Notify::Card::Moved`——经
    `UBattleGameFlowSubsystem::OnCardMoved`，见 `12` §3.1/§5.1）。
- **子系统交易队列**：
  - `PendingTransactions` + `ActiveTransaction`（`TWeakObjectPtr`，忙锁）已声明；
  - 驱动已通（09-10 复核，P2 §2-C 队列驱动缺陷①②③ 已消）：`EnqueueTransaction` 真正
  ...
  - **无嵌套栈字段**：`NestedStack` / `MaxNestedDepth` 未声明，`PushNestedTransaction` /
    ...
    拒绝分支，P5 首次使用（濒死打断）。
  - **无任何交易/窗口事件委托**：入队/收尾为直调（`Finish` 内调 `BroadcastFinish` +
    `OnTransactionFinished`），外部无可订阅点；事件化委托定义见 `12` §5.2 与附录 B。
- **`StartGame` 发牌** — 已按 §8 案例交易化编排：
  ...
  - 实牌唯一 Id 分配器已落地（`UCardManagerSubsystem::AllocateNewCard` =
    `FArkCard{InitializedCardCount++, Tags}`；计数在 `Deinitialize` 归零，09-10 九次复核）；
  - 驱动与容器写接口均已收口（P2 09-10 九次复核）→ **骨架侧无剩余阻塞**：`BroadcastFinish` 已真
...
public:
    ...
    void Drive();                 // 驱动器唯一入口（非虚）：DriveTransaction 调用；现直调 Execute()，P3 起改调 QueryTimings()
