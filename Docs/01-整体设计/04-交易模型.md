...
| **嵌套插队**（唯一插队口） | `PushNestedTransaction(NewTx)` | 挂起当前，**先完整结算子交易**，再恢复父交易 | 挂起（仍占 `ActiveTransaction`，不重入队） | 规则要求的立即打断（如濒死） |

**不变量**（P3 落字段、P5 首次使用）：

1. **同一时刻只有一个活动交易**：嵌套期间 `ActiveTransaction` 仍指向**父**交易（挂起态），
   子交易经 `NestedStack` 栈顶标识为"当前结算者"；外部观察不因嵌套改变（事件序见 `12` §6）。
2. **深度上限**：`MaxNestedDepth`（默认 1，可配）；超限**拒绝并告警**，不做隐式递归——
   与 §3.1 "不递归"原则一致，嵌套是显式栈而非函数递归。
...
TArray<FNestedTransactionFrame> NestedStack;   // 嵌套栈
int32 MaxNestedDepth = 1;                      // 深度上限（可配）

void PushNestedTransaction(UBattleTransaction* Child)
{
    if (!IsRunningOnServer() || !Child) return;
    if (NestedStack.Num() >= MaxNestedDepth) { /* 告警 + 拒绝 */ return; }
    ...
    ActiveTransaction = Child;                            // 子交易成为当前结算者
    Child->Start();                                       // 走统一门控（Validate → 入队）
    /* 或 Child->QueryTimings()（P3 后）：子交易同样开自己的窗口 */
}

void PopNestedTransaction(UBattleTransaction* Child)      // 由 OnTransactionFinished 在栈顶匹配时调用
{
    FNestedTransactionFrame Frame;
    NestedStack.Pop(Frame);
    ActiveTransaction = Frame.Parent;                     // 恢复父交易（不重发入队）
    /* 父交易从挂起点继续：窗口悬停 → 继续收响应；Execute 入口 → 继续落子 */
}
```

- **与窗口的关系**：父交易在 `Querying`（窗口悬停）时被嵌套，窗口**不关闭**（`FResponseWindow`
  以 `TStrongObjectPtr` 强持交易，见 `12` 附录 C）——子交易结算完后窗口继续询问/收响应。
- **与忙锁的关系**：`ActiveTransaction` 全程非空，`DriveTransaction` 的"忙则直接返回"逻辑不受影响
  （嵌套期间新入队仍只排队）。
- **决策点（P3 定）**：子交易的入队方式——`Start()` 会走 `EnqueueTransaction`（进 `PendingTransactions`），
...
- **窗内修改的通道分工（数值折叠 vs 结构直写）**：§2 的三类可改对象按字段形状各走各路，互不越界——
  - **数值 / 是否被防止**（`_Count`、`Amount`、`bIgnored`）→ **只经 `AppendModification`**（B 收口）：
    Op 集仅四类数值语义（Ignore/Add/Multiply/Clamp，§4 折叠公式），由族 `FoldMods` 在 `Execute`
  ...
  - **安全网 = `Execute` 开头的 `Validate()`**：窗内结构修改在落子前一律被复核（目标数/双端可解析/
    消息合法性），非法改动活不到落子；折叠结果（`bIgnored`、折叠后归零）在 `Validate` 之后才产生，
    由族在 `FoldMods` 后自判走 `Finish(false)`（§2 族模板第②步）。
...
- **`UCardMoveTransaction`**
  ...
  - 窗内干预点（§4.1 通道分工；P3 验收 §4.2）：`_Count` 只经 `ModRequest` 折叠——`FoldMods` 现为空实现直通，**折叠接线待落**（`P2` §2-D），接线后 `Execute` 于折叠处补自判（`bIgnored || _Count <= 0 → Finish(false)`）；`_From/_To/_Order/Predicate` 经 `GetMessage()`/`ModifyMessage()`（**待落**）。注意：预置牌（`Cards` 非空）的交易改 `_From` 会与源区脱钩——`HandleMovement_Selected` 查无牌即走"直送不 Consume"分支，牌凭空落目标区而源区不扣；窗内改 `_From` 前须确认交易为 DSL 取序，否则连 `Cards` 一并重填
- **`UDamageTransaction`**
...
- **`Battle/Transaction/BattleTransaction.h/.cpp`（+ `EventTypes.h`/`EventTags.h`）** —
  ...
  - `Execute()` 为**受保护** `PURE_VIRTUAL`（族钩子受保护：驱动器改经
    ...
    P3 开窗链只改 `Drive()` 体）、`FoldMods()` 为公开 `PURE_VIRTUAL`（族均必须实现）；
...
- **`UCardMoveTransaction`** — 独立文件（`Transaction/SubTransaction/CardMoveTransaction.*`）：
  ...
  - 族实现已落（`Execute` 执行骨架 + `Validate` + `FoldMods` 直通；`Execute` 受保护、
...
- **子系统交易队列**：
  ...
  - 驱动已通（P2 §2-C）：`EnqueueTransaction` 真正
    ...
    续驱（基类 `Finish` 内已调用）。
  - **无嵌套栈字段**：`NestedStack` / `MaxNestedDepth` 未声明，`PushNestedTransaction` /
    `PopNestedTransaction` 未落（`04` §3.2 的嵌套插队口整段缺失）——P3 落字段与入口 +
    拒绝分支，P5 首次使用（濒死打断）。
...
>
> 族钩子访问区（P2 §2-C 定案）：`Drive()` 为公开非虚驱动器唯一入口；族钩子 `Execute()`
> 受保护，另 `Validate/QueryTimings/OnWindowClosed/Finish/BroadcastFinish` 为 `protected`；
> `FoldMods()` 为公开族钩子（供族 `Execute` 开头调）。P3 开窗时驱动器只改 `Drive()` 体，
> 子系统不经保护成员。
...
> 与 `12` 附录 D 一致；P3 落字段（`UBattleGameFlowSubsystem`）。

```cpp
// 嵌套栈帧：一笔被挂起的父交易 + 正在结算的子交易（04 §3.2）
struct FNestedTransactionFrame
{
    TWeakObjectPtr<UBattleTransaction> Parent;   // 被挂起的父交易（仍占 ActiveTransaction，见 §3.2 不变量）
...
classDiagram
    ...
    class UCardMoveTransaction {
        <<移动族·落子型 ✅P2/P3>>
        -FMoveMessage Message 意图(From/To/Num/Order)
        -TArray~FArkCard~ Cards 预选入参/Execute后回写实际牌
        +Create(Msg, CardsToMove) 工厂
        #GetQueryTimingTags() 返回 Card.PreMove
        #Execute() 校验→FoldMods→源Consume→目标Add→Finish
        #FoldMods() 直通空实现(修正链待接·P2§2-D)
...
classDiagram
    ...
    class FNestedTransactionFrame {
        <<嵌套栈帧 🚧P5·定义见12附录D>>
        +TWeakObjectPtr~UBattleTransaction~ Parent
        +TWeakObjectPtr~UBattleTransaction~ Child
    }
    ...
    FNestedTransactionFrame --> UBattleTransaction : Parent/Child弱引用
